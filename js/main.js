$(document).ready(function() {
	...
	// .nain-section reduce opacity when scrolling down
	$(window).scroll(function(){
		if($(window).width() > 1260) {
			windowScroll = $(window).scrollTop();
			contentOpacity = 1 - (windowScroll / ($('#main').offset().top+$('#main').height()));
			$('.main-section').css('transform','translateY('+Math.floor(windowScroll*0.16)+'px)');
			$('.main-section').css('-webkit-transform','translateY('+Math.floor(windowScroll*0.16)+'px)');
			$('.main-section').css('opacity',contentOpacity.toFixed(2));
		}
	});
